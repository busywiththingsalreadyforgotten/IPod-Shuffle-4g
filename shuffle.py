...
Based on: https://github.com/nims11/IPod-Shuffle-4g

Known bugs:
- I think it still crashes if tracks are added with no artist specified in their mp3 tags
...
def validate_unicode(path):
  ...
  return (os.sep).join(path_list) + (extension if last_raise and extension in audio_ext else '')
...
class Text2Speech(object):
  ...
  def text2speech(out_wav_path, text):
    ...
    Text2Speech.balabolka(out_wav_path, text)
  ...
  @staticmethod
  def balabolka(out_wav_path, unicodetext):
    
    balabolkaPath = os.path.dirname(os.path.realpath(__file__))
    balabolkaPath = os.path.join(balabolkaPath, "balabolka_console")
    balabolkaPath = os.path.join(balabolkaPath, "balabolka_console.exe")
    lang = detect(unicodetext)
    if lang == "de":
      cmd = [balabolkaPath, "-n", "Microsoft Hedda Desktop", "-t", unicodetext, "-w", out_wav_path]
    else:
      cmd = [balabolkaPath, "-n", "Microsoft Zira Desktop", "-t", unicodetext, "-w", out_wav_path]
...
class Record(object):
  ...
  def text_to_speech(self, text, dbid, playlist = False):
    global g_speakable_trackFiles_array 
    if self.voiceover:
      ...
      if os.path.isfile(path) == False:
        ...
        #print "speakable file added"

      g_speakable_wavFiles_array.append(path)
  ...
  def path_to_ipod(self, filename):
    ...
    ipodname = (os.sep).join(os.path.abspath(filename)[baselen:].split(os.path.sep))
    return ipodname

  def ipod_to_path(self, ipodname):
    return os.path.abspath(os.path.join(self.base, os.path.sep.join(ipodname.split(os.sep))))
...
class Playlist(Record):
  ...
  def remove_relatives(self, relative, filename):
    ...
    relPath = fullPath[fullPath.index(ipodpath)+len(ipodpath):].lower()
...
class Shuffler(object):
  ...
  def initialize(self):
    ...
    #for dirname in (('iPod_Control' + os.sep + 'Speakable' + os.sep + 'Playlists'), ('iPod_Control' + os.sep + 'Speakable' + os.sep + 'Tracks')):
    ...
    for dirname in (('iPod_Control' + os.sep + 'iTunes'), ('iPod_Control' + os.sep + 'Music'), ('iPod_Control' + os.sep + 'Speakable' + os.sep + 'Playlists'), ('iPod_Control' + os.sep + 'Speakable' + os.sep + 'Tracks')):
  ...
  def populate(self):
    ...
    for (dirpath, dirnames, filenames) in os.walk(self.path):
      ...
      if ("ipod_control" + os.sep + "speakable") not in dirpath.lower() and (os.sep + ".") not in dirpath.lower():
        for filename in sorted(filenames, key = lambda x: x.lower()):
          fullPath = os.path.abspath(os.path.join(dirpath, filename))

          ## Modified here, initial code was trimming the first character off the tracknames
          #relPath = fullPath[fullPath.index(self.path)+len(self.path)+1:].lower()
          relPath = fullPath[fullPath.index(self.path)+len(self.path):].lower()
...
def createPlaylistPerArtist(path):
  ...
  for (dirpath, dirnames, filenames) in os.walk(path):
    ...
    if ("ipod_control" + os.sep + "speakable") not in dirpath.lower() and (os.sep + ".") not in dirpath.lower():
      for filename in sorted(filenames, key = lambda x: x.lower()):
        fullPath = os.path.abspath(os.path.join(dirpath, filename))
        #relPath = fullPath[fullPath.index(result.path)+len(result.path):].lower()
        #fullPath = os.path.abspath(os.path.join(result.path, relPath));
        if os.path.splitext(filename)[1].lower() in (".mp3", ".m4a", ".m4b", ".m4p", ".aa", ".wav"):
          audio = mutagen.File(fullPath, easy = True)
          info = "%s,%s - %s" % (int(audio.info.length),audio['artist'][0],audio['title'][0])
          if not audio['artist'][0] in playLists:
            # create new playlist for artist
            playLists[audio['artist'][0]] = {"tracks":[fullPath], "trackInfo":[info]}
          else:  
            # add to existing artist playlist
            playLists[audio['artist'][0]]['tracks'].append(fullPath)
            playLists[audio['artist'][0]]['trackInfo'].append(info)

  # write playlists to ipod
  ipodPath = path
  if ipodPath[(len(ipodPath)-1):] != os.sep:
    ipodPath = ipodPath + os.sep
  for key, value in playLists.items():
    f = open( (os.path.join(ipodPath, key) + ".m3u"),"w") #opens file with name of "test.txt"
    f.write("#EXTM3U\n")
    for i in range(0, len(value["tracks"])):
      s = "#EXTINF:%s\n" % (value["trackInfo"][i])
      #f.write(s)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--create-playlists', action='store_true', help='Automatically create one playlist per artist')
  ...
  # delete old playlists that are in the root
  for f in os.listdir(result.path):
    if f.endswith('.m3u'):
      os.remove(os.path.join(result.path, f))

  # create new playlists
  if result.create_playlists:
    createPlaylistPerArtist(result.path)
  ...
  # Delete unused speakable files
  for dirname in (((result.path).upper() + os.sep + 'iPod_Control' + os.sep + 'Speakable' + os.sep + 'Playlists'), ((result.path).upper() + os.sep + 'iPod_Control' + os.sep + 'Speakable' + os.sep + 'Tracks')):
    for f in os.listdir(dirname):
      filepath = os.path.join(dirname, f)
      if not filepath in g_speakable_wavFiles_array:
        # delete
        os.remove(filepath)
        print '[-] Removing old speakable file %s' % filepath
